class ParkScene {
    ...
    createRoadMarking(length, isDashed = false) {
        const canvas = document.createElement('canvas');
        canvas.width = 256;
        canvas.height = 32;
        const ctx = canvas.getContext('2d');
    ...
    createRoadNetwork() {
        ...
        const createStraightRoad = (x, z, width, length, rotation) => {
            ...
            if (width >= this.MAIN_ROAD_WIDTH) {
                ...
                centerLine.position.set(x, 0.151, z);
            ...
            if (rotation === 0) {
                edge1.position.set(x, 0.151, z + width / 2 - 0.1);
                edge2.position.set(x, 0.151, z - width / 2 + 0.1);
            } else {
                edge1.position.set(x + width / 2 - 0.1, 0.151, z);
                edge2.position.set(x - width / 2 + 0.1, 0.151, z);
            }
        ...
        const createIntersection = (x, z, size) => {
            const intersectionGeo = new THREE.BoxGeometry(size, 0.15, size);
            const intersection = new THREE.Mesh(intersectionGeo, roadMat);
            intersection.position.set(x, 0.075, z);
            intersection.receiveShadow = true;
            roadGroup.add(intersection);
            
            const crosswalkSize = 6;
            const crosswalkGeo = new THREE.PlaneGeometry(crosswalkSize, 3);
            ...
            positions.forEach(pos => {
                ...
                crosswalk.position.set(x + pos.x, 0.152, z + pos.z);
        ...
        const intersectionSize = this.MAIN_ROAD_WIDTH + 2;

        createStraightRoad(0, offset, this.MAIN_ROAD_WIDTH, offset * 2, 0);
        createStraightRoad(0, -offset, this.MAIN_ROAD_WIDTH, offset * 2, 0);
        createStraightRoad(offset, 0, this.MAIN_ROAD_WIDTH, offset * 2, Math.PI / 2);
        createStraightRoad(-offset, 0, this.MAIN_ROAD_WIDTH, offset * 2, Math.PI / 2);

        createIntersection(offset, offset, intersectionSize);
        createIntersection(-offset, offset, intersectionSize);
        createIntersection(offset, -offset, intersectionSize);
        createIntersection(-offset, -offset, intersectionSize);
        ...
        createIntersection(secondaryOffset, secondaryOffset, this.SECONDARY_ROAD_WIDTH + 1);
        createIntersection(-secondaryOffset, secondaryOffset, this.SECONDARY_ROAD_WIDTH + 1);
        createIntersection(secondaryOffset, -secondaryOffset, this.SECONDARY_ROAD_WIDTH + 1);
        createIntersection(-secondaryOffset, -secondaryOffset, this.SECONDARY_ROAD_WIDTH + 1);
        ...
        const createSidewalk = (x, z, w, l, rot) => {
            const sidewalkGeo = new THREE.BoxGeometry(l, 0.2, w);
        ...
        createSidewalk(0, offset + this.MAIN_ROAD_WIDTH / 2 + sidewalkWidth / 2 + 0.5, sidewalkWidth, offset * 2, 0);
        createSidewalk(0, -offset - this.MAIN_ROAD_WIDTH / 2 - sidewalkWidth / 2 - 0.5, sidewalkWidth, offset * 2, 0);
        createSidewalk(offset + this.MAIN_ROAD_WIDTH / 2 + sidewalkWidth / 2 + 0.5, 0, sidewalkWidth, offset * 2, Math.PI / 2);
        createSidewalk(-offset - this.MAIN_ROAD_WIDTH / 2 - sidewalkWidth / 2 - 0.5, 0, sidewalkWidth, offset * 2, Math.PI / 2);
    ...
    createCar(x, z, rotation, type = 'sedan', color = 0xe74c3c) {
        ...
        carGroup.userData = {
            ...
            vehicleType: type,
            wheels: wheels,
            speed: type === 'sedan' ? 0.08 : 0.05,
            baseSpeed: type === 'sedan' ? 0.08 : 0.05,
            currentSpeed: type === 'sedan' ? 0.08 : 0.05,
            direction: rotation,
            lane: 0,
            isWaiting: false,
            waitTimer: 0
    ...
    createVehicles() {
        const offset = this.ROAD_OFFSET;
        const secondaryOffset = offset / 2;
        const roadWidth = this.MAIN_ROAD_WIDTH;
        
        const colors = [0xe74c3c, 0x3498db, 0x2ecc71, 0xf39c12, 0x9b59b6, 0x1abc9c, 0xe91e63];
        
        const vehicleConfigs = [
            { x: -30, z: offset + 2, rot: 0, type: 'sedan', color: colors[0] },
            { x: 10, z: offset + 2, rot: 0, type: 'sedan', color: colors[1] },
            { x: 40, z: offset - 2, rot: Math.PI, type: 'sedan', color: colors[2] },
            { x: -20, z: offset - 2, rot: Math.PI, type: 'bus', color: 0xffd700 },
            { x: 30, z: -offset + 2, rot: 0, type: 'sedan', color: colors[3] },
            { x: -40, z: -offset + 2, rot: 0, type: 'sedan', color: colors[4] },
            { x: 20, z: -offset - 2, rot: Math.PI, type: 'bus', color: 0x4169e1 },
            
            { x: offset - 2, z: 30, rot: -Math.PI / 2, type: 'sedan', color: colors[5] },
            { x: offset - 2, z: -20, rot: -Math.PI / 2, type: 'sedan', color: colors[6] },
            { x: offset + 2, z: 10, rot: Math.PI / 2, type: 'sedan', color: colors[0] },
            { x: offset + 2, z: -40, rot: Math.PI / 2, type: 'bus', color: 0x32cd32 },
            { x: -offset + 2, z: 20, rot: -Math.PI / 2, type: 'sedan', color: colors[1] },
            { x: -offset + 2, z: -30, rot: -Math.PI / 2, type: 'sedan', color: colors[2] },
            { x: -offset - 2, z: -10, rot: Math.PI / 2, type: 'sedan', color: colors[3] },
            
            { x: -secondaryOffset + 1.5, z: secondaryOffset + 1.5, rot: -Math.PI / 2, type: 'sedan', color: colors[4] },
            { x: secondaryOffset - 1.5, z: -secondaryOffset - 1.5, rot: Math.PI / 2, type: 'sedan', color: colors[5] }
        ];
        
        vehicleConfigs.forEach(config => {
            const vehicle = this.createCar(config.x, config.z, config.rot, config.type, config.color);
    ...
    getTrafficLightStateAtPosition(x, z, direction) {
        const offset = this.ROAD_OFFSET;
        const secondaryOffset = offset / 2;
        const thresholds = [offset, secondaryOffset, -secondaryOffset, -offset];
        const threshold = 12;
        
        for (let i = 0; i < this.trafficLights.length; i++) {
            const light = this.trafficLights[i];
            const lx = light.position.x;
            const lz = light.position.z;
            
            if (Math.abs(x - lx) < threshold && Math.abs(z - lz) < threshold) {
    ...
    updateVehicles(time, delta) {
        ...
        this.vehicles.forEach(vehicle => {
            ...
            const dx = Math.sin(data.direction) * speed;
            const dz = Math.cos(data.direction) * speed;
            
            let shouldStop = false;
            
            const testX = vehicle.position.x + dx * stopLine;
            const testZ = vehicle.position.z + dz * stopLine;
            const lightState = this.getTrafficLightStateAtPosition(testX, testZ, data.direction);
            
            if (lightState === 'red' || lightState === 'yellow') {
                const nearIntersection = this.trafficLights.some(light => {
                    const dist = Math.sqrt(
                        Math.pow(vehicle.position.x - light.position.x, 2) +
                        Math.pow(vehicle.position.z - light.position.z, 2)
                    );
                    return dist < stopLine + 5;
                });
            ...
            for (let other of this.vehicles) {
                ...
                const dist = Math.sqrt(
                    Math.pow(vehicle.position.x - other.position.x, 2) +
                    Math.pow(vehicle.position.z - other.position.z, 2)
                );
                
                if (dist < safeDistance) {
                    const angleToOther = Math.atan2(
                        other.position.x - vehicle.position.x,
                        other.position.z - vehicle.position.z
                    );
                    const angleDiff = Math.abs(((angleToOther - data.direction + Math.PI * 3) % (Math.PI * 2)) - Math.PI);
            ...
            if (shouldStop) {
                data.currentSpeed = Math.max(0, data.currentSpeed - delta * 0.2);
            } else {
                data.currentSpeed = Math.min(data.baseSpeed, data.currentSpeed + delta * 0.1);
            }
            
            if (data.currentSpeed > 0.001) {
                vehicle.position.x += Math.sin(data.direction) * data.currentSpeed;
                vehicle.position.z += Math.cos(data.direction) * data.currentSpeed;
                
                data.wheels.forEach(wheel => {
                    wheel.rotation.x += data.currentSpeed * 3;
                });
            }
            
            if (vehicle.position.x > boundary) vehicle.position.x = -boundary;
            if (vehicle.position.x < -boundary) vehicle.position.x = boundary;
            if (vehicle.position.z > boundary) vehicle.position.z = -boundary;
            if (vehicle.position.z < -boundary) vehicle.position.z = boundary;
        });
    }
